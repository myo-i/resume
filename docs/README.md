...
- **プロジェクトでの課題**一個を詳細に書いたほうが良いかも！！（複数問われる可能性もあるからそれは控えとして持っとく）
    - **①技術のキャッチアップ**  
    ...
    - **対応していない予約語の対応**
    COBOL から Java への変換の際、変換ツールが CSV 形式のファイルの読み書きに対応していないという問題が発生していました
    そこでチームの対応としては COBOL で CSV 形式のファイルの読み書きを再現するという方針を決定し、私はその再現を任され、以下のような対応を行いました。
    - COBOLで1つの変数に1レコード格納
    - 特定のデリミタで区切り、区切ったデータをそれぞれ変数に格納し、プログラム内で仕様する
    - 処理が終わりCSV形式で書き込む際は、必要な変数を全てデリミタで結合し、1つの変数に格納し書き込み処理を行う。
    しかし、
    - COBOL コードの肥大化に伴い、Java コードが肥大化し、コンパイルが通らないケースが増加した。
    - COBOL では変数は固定長のため、変数の宣言の際にデータのサイズを指定する必要があるが。①データのサイズの計算に時間がかかる②データサイズの自動計算スクリプトを作ろうにも、データの入れ子を表現するのが困難な上に、同じ型の宣言でも複数の宣言方法がある。などの問題が発生したため COBOL での再現を中止しました。

    そこで次に考案されたのが、COBOL で CSV の入出力を行うプログラムを呼び出す処理を書き、プログラム自体は Java で実装するというものでした。
    私は CSV の読み込み部分を担当しました。（プログラムには BY REFERENCE 句を用いることで渡したデータを書き込んでもらうようにした）ここでの問題は
    - ツールを用いて COBOLを Java に変換した際、Java の中身はツール独自の型やメソッドのみが用いられており、独自の引数の受け渡しを前提としたプログラムを書く必要があった。（必要なのはデータを持った変数とそれを分割して受け取る変数）
